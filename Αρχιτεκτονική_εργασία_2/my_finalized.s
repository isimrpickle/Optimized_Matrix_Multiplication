...
#loading adresses of the arrays into registers
	la $t4, array_x
	la $t0, array_y
	la $t5, array_z
	li $s0, 32 #size of the array

	addi $t6,$zero, 0 #index i
	addi $t7,$zero, 0 #index k
	addi $t8,$zero, 0 #index j

loop_i:
	beq $t6, $s0, out #if we have iterated every row we finish the execution
	addi $t7,$zero, 0 #resetting k

loop_k:
	beq $t7, $s0, incrementing_i #if k==$s0(the size of the array) we go to the next row
	addi $t8,$zero, 0 #resetting j

# Calculate the index [i][k] in order to load array x[i][k]
	mul $s2, $t6, $s0    #i* n #(for example the first element of the 2nd line is 1*32=32th element)
	add $s2, $s2, $t7    #i* n+ k 
	sll $s2, $s2, 2      #(i* n+ k)* 4 (1 word is 4 bytes)
	add $s2, $s2, $t4    #adding to the initial offset the [i][k] index
	lw $s3, 0($s2)       #loading into register $s3 the value of x[i][k]

loop_j:
	beq $t8, $s0, incrementing_k

# calculating the index of y[k][j] with the same logic as before.
	mul $s1, $t7, $s0    #k* n
	add $s1, $s1, $t8    #k* n+ j
	sll $s1, $s1, 2      #(k* n+ j) * 4
	add $s1, $s1, $t0    #initial offset + offset for B[k][j]
	lw $s4, 0($s1)       #loading B[k][j] to $s4

# 	now for  y[i][j] address and load current value
	mul $s1, $t6, $s0    #i* n
	add $s1, $s1, $t8    #i* n+ j
	sll $s1, $s1, 2      #(i* n+ j)* 4
	add $s1, $s1, $t5    #Initial offset + offset for C[i][j]
	lw $s5, 0($s1)       # C[i][j] to $s5

#  multiplication with overflow check
	mult $s3, $s4        # A[i][k] * B[k][j]
	mflo $s6             # getting the low bits of the multiplication results
	mfhi $s7             # higher bits of the multiplication result
	sra $t9, $s6, 31     # Get sign of low word (the first bit shows if it's positive(1)or negative(0)
	beq $t9, $s7, no_overflow    # the high word will only have the same sign bit as the low word. So if it's not the same that means we're dealing with a very big/small number so an overflow has occured

	j overflow #else jump to overflow

no_overflow:
	add $s5, $s5, $s6    # C[i][j] += A[i][k] * B[k][j]
